...
export const CheckoutCard = async ({ cart }: { cart: Stripe.Checkout.Session }) => {
	const t = await getTranslations("/cart.page");
	const locale = await getLocale();

		const allProductsDigital = cart.line_items?.data.every((line) => {
		const product = line.price?.product;
		return typeof product === "object" && "shippable" in product ? !product.shippable : false;
	});
	...
	return (
		<section className="max-w-md pb-12">
			...
			<StripePayment
				...
				shippingRates={[]}
				allProductsDigital={allProductsDigital||false}
				locale={locale}
			/>
		</section>
	);
